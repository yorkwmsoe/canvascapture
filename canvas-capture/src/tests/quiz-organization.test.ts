...
describe('Testing assembleQuizQuestionsAndComments', async () => {
    ...
    test('assembleQuizQuestionsAndComments', async () => {
        const output = await assembleQuizQuestionsAndComments(auth, sampleCourse, sampleAssignment, sampleSubmission)
        expect(output).toEqual(expectedOutput)
    })
})

describe('Testing formatQuizQuestions', async () => {
    const questionsDataString = JSON.stringify([{"quiz_id":2,"question_name":"Question","question_description":"<p>Drake????</p>","position":1,"points_possible":1,"correct_comments":"<p>Wow you are so amazing you guessed this correct, these are the corrents</p>","neutral_comments":"<p>To Drake or not to drake, these are neutral comment</p>","incorrect_comments":"<p>You are a failure, these are the incorrect comments</p>","correct_answers":[],"correct":true,"question_type":"true_false_question"},{"quiz_id":2,"question_name":"Question","question_description":"<p>Nice Spice???</p>","position":2,"points_possible":1,"correct_comments":"","neutral_comments":"","incorrect_comments":"","correct_answers":[],"correct":true,"question_type":"multiple_choice_question"},{"quiz_id":2,"question_name":"Question","question_description":"<p>This is a multiple answers question and is being used to test the presence, of correct, incorrent, neutral, and additional comments</p>","position":3,"points_possible":1,"correct_comments":"<p>You guessed correctly DRAKE, these are the correct comments</p>","neutral_comments":"<p>These are the Neutral comments Drake celly</p>","incorrect_comments":"<p>You guessed incorrectly DRAKE, these are the incorrect comments</p>","correct_answers":[],"correct":false,"question_type":"multiple_answers_question"}])
