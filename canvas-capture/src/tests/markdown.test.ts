...
describe('markdown convertToHeader', () => {
    ...
    //Act and Assert
    test.each(listOfHeaderSizes)(
        'convertToHeader is same as SampleHeader',
        (head) => {
...
describe('Create list', () => {
    ...
    test.each(correctLists)('Testing lists', (listCheck) => {
        const checkList: string = createList(
            ['one', 'two', 'three'],
            listCheck.bullet
        )
...
describe('Testing createlink', () => {
    const ikeaLink = createLink('This Link to Ikea', 'Link', 'https://ikea.com')
...
describe('Test Table Header', () => {
    ...
    test('Table Header ', () => {
        const returnedTableHeader = createTableHeader([
            'head1',
            'head2',
            'head3',
        ])
