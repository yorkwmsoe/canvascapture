...
import { convertToHeader, createList, createTableHeader, createTableRows, writeToFile } from './markdown'
import { mkdirSync } from 'fs'

const basePath = 'output'

export function generateAssignment(course: Course, assignment: Assignment, submission: Submission, score: 'high' | 'median' | 'low', commit: boolean = true) {
    const folderPath = `${basePath}/${course.name}/${assignment.name}`
    mkdirSync(folderPath, { recursive: true })

    const items: string[] = [...assembleTitleAndGrade(assignment, submission), ...assembleFeedbackInfo(submission), ...assembleDescriptionInfo(assignment), ...assembleRubricInfo(assignment, submission), ...assembleSubmissionInfo(submission)]
