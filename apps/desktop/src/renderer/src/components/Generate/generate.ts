...
export async function generate(
  ...
  await rm(join(documentsPath, sanitizePath(generationName)), { recursive: true, force: true });
  ...
  for (const course of courses) {
    ...
    const coursePath = sanitizePath(join(generationName, getCourseName(course)));
  ...
  const htmlData: FilePathContentPair[] = Object.keys(courseMarkdownContent).map((courseName) => {
    const markdownContent = courseMarkdownContent[courseName];
    const filePath = join(documentsPath, generationName, courseName, `${courseName}.md`);
