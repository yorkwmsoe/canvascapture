...
function createAverageGradeDataSet(
    ...
    for (const assignmentGroup of assignmentGroups) {
        ...
        const submissions = groupSubmissionsMap.get(assignmentGroup.id)!
            .filter((submission) => {
                return !submission.missing && !submission.late && submission.grade !== null
            })
...
function calculateGradingTurnaroundStatistics(
    ...
    const gradingTurnaround = submissions.map((submission) => {
        if (!submission.graded_at || !assignmentDueDateMap.has(submission.assignment_id))
            return null;
...
function calculateBusinessDays(startDate: Date, endDate: Date): number {
    ...
    while (currentDate.getTime() + 24 * 60 * 60 * 1000 <= normalizedEndDate.getTime()) {
